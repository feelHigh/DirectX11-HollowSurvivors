...
{
	HHPathMgr::init();
	wstring strProjPath = HHPathMgr::GetProjectPath();
	wstring strCppPath = strProjPath + L"\\Scripts\\HHScriptMgr.cpp";
	wstring strHeaderPath = strProjPath + L"\\Scripts\\HHScriptMgr.h";

	// 1. 현재 존재하는 모든 스크립트를 알아내야함.
	wstring strScriptIncludePath = HHPathMgr::GetIncludePath();
	...
	fwprintf_s(pFile, L"class HHScript;\n\n");

	fwprintf_s(pFile, L"class HHScriptMgr\n{\n");
	fwprintf_s(pFile, L"public:\n\tstatic void GetScriptInfo(vector<wstring>& _vec);\n");
	fwprintf_s(pFile, L"\tstatic HHScript * GetScript(const wstring& _strScriptName);\n");
	fwprintf_s(pFile, L"\tstatic HHScript * GetScript(UINT _iScriptType);\n");
	fwprintf_s(pFile, L"\tstatic const wchar_t * GetScriptName(HHScript * _pScript);\n};\n");
	...
	fwprintf_s(pFile, L"#include \"HHScriptMgr.h\"\n\n");
	...
	// 두번째 함수 작성
	fwprintf_s(pFile, L"HHScript * HHScriptMgr::GetScript(const wstring& _strScriptName)\n{\n");
	...
	// 세번째 함수
	fwprintf_s(pFile, L"HHScript * HHScriptMgr::GetScript(UINT _iScriptType)\n{\n");
	...
	// 네번째 함수
	fwprintf_s(pFile, L"const wchar_t * HHScriptMgr::GetScriptName(HHScript * _pScript)\n{\n");
