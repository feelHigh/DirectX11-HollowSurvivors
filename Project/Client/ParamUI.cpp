...
#include "TreeUI.h"
bool ParamUI::InputTexture(Ptr<HHTexture>& _CurTex, const string& _Desc, EditorUI* _Inst, DELEGATE_1 _MemFunc)
{
	Ptr<HHTexture> CurTex = _CurTex;
...
bool ParamUI::InputPrefab(Ptr<HHPrefab>& _CurPrefab, const string& _Desc, EditorUI* _Inst, DELEGATE_1 _MemFunc)
{
	Ptr<HHPrefab> CurPrefab = _CurPrefab;
	...
	if (ImGui::BeginDragDropTarget())
	{
		...
		if (payload)
		{
			...
			{
				_CurPrefab = ((HHPrefab*)pAsset.Get());
			}
		}
	...
	if (ImGui::Button(szID, ImVec2(18.f, 18.f)))
	{
		...
		HHAssetMgr::GetInstance()->GetAssetNames(ASSET_TYPE::PREFAB, vecPrefabName);
