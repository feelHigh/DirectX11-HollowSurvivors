...
void ScriptUI::Update()
{
	...
	wstring strScriptName = HHScriptMgr::GetScriptName(m_Script);
	...
	for (size_t i = 0; i < vecParam.size(); ++i)
	{
		...
		{
			Ptr<HHTexture>& pTex = *((Ptr<HHTexture>*)vecParam[i].pData);
		...
		{
			Ptr<HHPrefab>& pPrefab = *((Ptr<HHPrefab>*)vecParam[i].pData);
...
void ScriptUI::SetTargetScript(HHScript* _Script)
{
	m_Script = _Script;
...
void ScriptUI::SelectPrefab(DWORD_PTR _ListUI)
{
	...
	Ptr<HHPrefab> pPrefab = HHAssetMgr::GetInstance()->FindAsset<HHPrefab>(strAssetName);
