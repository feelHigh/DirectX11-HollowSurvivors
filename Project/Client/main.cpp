    ...
    _In_ int       nCmdShow)
{
    ...
    // HHEngine 객체 초기화
    if (FAILED(HHEngine::GetInstance()->Initialize(hWnd
                                                , POINT{ 1600, 900 }
                                                , (OBJECT_SAVE)&HHLevelSaveLoad::SaveGameObject
                                                , (OBJECT_LOAD)&HHLevelSaveLoad::LoadGameObject)))
    {
        MessageBox(nullptr, L"HHEngine 초기화 실패", L"엔진 초기화 실패", MB_OK);
        return 0;
    }
    ...
    while (true)
    {
        ...
        {
            HHEngine::GetInstance()->Progress();

#ifdef _DEBUG
            HHEditorMgr::GetInstance()->Tick();
#endif

            HHDevice::GetInstance()->Present();
        }
    }
