...
void CameraUI::Update()
{
    ...
    HHCamera* pCam = GetTargetObject()->Camera();
...
void CameraUI::LayerCheck()
{
    ...
    ImGui::SameLine(100);
    if (ImGui::Button("Show##LayerCheckBtn", ImVec2(80.f, 18.f)))
    {
    ...
    if (ImGui::BeginTable("##LayerCheckTable", 2, ImGuiTableFlags_Resizable | ImGuiTableFlags_NoSavedSettings | ImGuiTableFlags_Borders))
    {
        for (int i = 0; i < 32; i++)
        {
            ...
            wstring wlayerName = HHLevelMgr::GetInstance()->GetCurrentLevel()->GetLayer(i)->GetName();
