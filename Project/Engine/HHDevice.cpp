...
int HHDevice::Initialize(HWND _hWnd, UINT _Width, UINT _Height)
{
    // 윈도우 핸들 및 해상도 값 저장
    m_hWnd = _hWnd;
    ...
    {
        MessageBox(nullptr, L"Constant Buffer 생성 실패", L"장치 초기화 실패", MB_OK);
        return E_FAIL;
    }
    ...
    {
        MessageBox(nullptr, L"Depth Stencil State 생성 실패", L"장치 초기화 실패", MB_OK);
        return E_FAIL;
    }
...
int HHDevice::CreateView()
{
    ...
    // RenderTarget 텍스쳐 생성
    m_RTTex = HHAssetMgr::GetInstance()->CreateTexture(L"RenderTargetTex", RenderTargetTex);

    // DepthStencil 텍스쳐 생성
    m_DSTex = HHAssetMgr::GetInstance()->CreateTexture(L"DepthStencilTex"
...
int HHDevice::CreateConstantBuffer()
{
    HHConstantBuffer* pCB = nullptr;

    // 월드, 뷰, 투영 행렬 전달
    pCB = new HHConstantBuffer;
    ...
    pCB = new HHConstantBuffer;
    if (FAILED(pCB->Create(CONSTANT_BUFFER_TYPE::MATERIAL, sizeof(tMaterialConstant))))
    {
        MessageBox(nullptr, L"상수버퍼 생성 실패", L"초기화 실패", MB_OK);
        return E_FAIL;
    }
...
int HHDevice::CreateRasterizerState()
{
...
int HHDevice::CreateDepthStencilState()
{
...
int HHDevice::CreateSamplerState()
{
