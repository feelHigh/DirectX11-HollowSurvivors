...
void CreateObject(HHGameObject* _NewObject, int _LayerIndex)
{
	tTask task = {};
	...
	HHTaskMgr::GetInstance()->AddTask(task);
}
...
void ChangeLevel(HHLevel* _Level, LEVEL_STATE _NextLevelState)
{
	tTask task = {};
...
void DrawDebugRect(Vec3 _Pos, Vec3 _Scale, Vec3 _Rot, Vec4 _Color, float _Life, bool _DepthTest)
{
	...
	HHRenderMgr::GetInstance()->AddDebugShapeInfo(Info);
}
