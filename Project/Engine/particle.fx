...
VS_OUT VS_Particle(VS_IN _in)
{
    ...
    output.vLocalPosition = _in.vPosition;
...
void GS_Particle(point VS_OUT _in[1], inout TriangleStream<GS_OUT> _OutStream)
{
    ...
    tParticle Particle = ParticleBuffer[_in[0].InstanceID];
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        output[i].vPosition = mul(output[i].vPosition, matProjection);
        output[i].InstanceID = _in[0].InstanceID;
    }
    ...
    if (Module.VelocityAlignment)
    {
        for (int i = 0; i < 4; ++i)
        {
            ...
            output_cross[i].vPosition = mul(output_cross[i].vPosition, matProjection);
            output_cross[i].InstanceID = _in[0].InstanceID;
        }
...
float4 PS_Particle(GS_OUT _in) : SV_Target
{
    ...
    if (g_btex_0)
    {
        ...
        vParticleColor *= ParticleBuffer[_in.InstanceID].vColor;
    }
