...
void HHKeyMgr::Tick()
{
	...
	if (HHEngine::GetInstance()->GetMainWnd() == GetFocus())
	{
		...
		if (m_MouseCapture)
		{
			...
			ScreenToClient(HHEngine::GetInstance()->GetMainWnd(), &ptMousePos);
			...
			ClientToScreen(HHEngine::GetInstance()->GetMainWnd(), &ptCapturePos);
