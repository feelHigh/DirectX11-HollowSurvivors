...
void HHTexture::Binding(UINT _RegisterNum)
{
...
void HHTexture::Binding_CS_SRV(UINT _RegisterNum)
{
	m_RecentBindingRegisterNum = _RegisterNum;
	CONTEXT->CSSetShaderResources(_RegisterNum, 1, m_SRV.GetAddressOf());
}

void HHTexture::Binding_CS_UAV(UINT _RegisterNum)
{
	UINT i = -1;
...
void HHTexture::Clear(UINT _RegisterNum)
{
...
int HHTexture::Load(const wstring& _FilePath)
{
...
int HHTexture::Save(const wstring& _FilePath)
{
	return 0;
}
...
int HHTexture::Create(UINT _Width, UINT _Height, DXGI_FORMAT _PixelFormat, UINT _Flags, D3D11_USAGE _Usage)
{
...
int HHTexture::Create(ComPtr<ID3D11Texture2D> _Tex2D)
{
	m_Tex2D = _Tex2D;
	m_Tex2D->GetDesc(&m_Desc);
